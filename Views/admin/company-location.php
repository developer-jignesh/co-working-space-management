...
<h2><?php echo $edit_mode ? __('Edit Company Location', 'coworking-text-domain') : __('Add New Company Location', 'coworking-text-domain'); ?>
</h2>

<form method="post">
    <?php wp_nonce_field('manage_company_location_nonce', 'company_location_nonce'); ?>
    <input type="hidden" name="company_id"
        value="<?php echo esc_attr($company_location_controller->get_data('company_id') ?? ''); ?>">
    <input type="hidden" name="location_id"
        value="<?php echo esc_attr($company_location_controller->get_data('location_id') ?? ''); ?>">
    ...
    <table class="form-table">
        ...
        <tr>
            ...
            <td><input type="number" name="leased_space" id="leased_space"
                    value="<?php echo esc_attr($company_location_controller->get_data('leased_space') ?? ''); ?>"
                    required></td>
        </tr>
        <tr>
            <th><label for="monthly_rent"><?php _e('Monthly Rent', 'coworking-text-domain'); ?></label></th>
            <td><input type="text" name="monthly_rent" id="monthly_rent"
                    value="<?php echo esc_attr($company_location_controller->get_data('monthly_rent') ?? ''); ?>"
                    required></td>
        </tr>
        <tr>
            <th><label for="contract_start_date"><?php _e('Contract Start Date', 'coworking-text-domain'); ?></label>
            </th>
            <td><input type="date" name="contract_start_date" id="contract_start_date"
                    value="<?php echo esc_attr($company_location_controller->get_data('contract_start_date') ?? ''); ?>"
                    required></td>
        </tr>
        <tr>
            <th><label for="contract_end_date"><?php _e('Contract End Date', 'coworking-text-domain'); ?></label></th>
            <td><input type="date" name="contract_end_date" id="contract_end_date"
                    value="<?php echo esc_attr($company_location_controller->get_data('contract_end_date') ?? ''); ?>"
                    required></td>
        </tr>
    </table>

    <p>
        <input type="submit" name="<?php echo $edit_mode ? 'update_company_location' : 'add_company_location'; ?>"
            value="<?php echo $edit_mode ? __('Update Company Location', 'coworking-text-domain') : __('Add Company Location', 'coworking-text-domain'); ?>"
            class="button button-primary">

        <?php if ($edit_mode): ?>
            <a href="<?php echo admin_url('admin.php?page=manage-company-locations'); ?>"
                class="button"><?php _e('Reset Form', 'coworking-text-domain'); ?></a>
        <?php endif; ?>
    </p>
</form>
    ...
    <tbody>
        <?php foreach ($company_locations as $company_location): ?>
            <tr>
                ...
                <td>
                    <a href="<?php echo admin_url('admin.php?page=manage-company-locations&action=edit&company_id=' . esc_attr($company_location->company_id) . '&location_id=' . esc_attr($company_location->location_id)); ?>"
                        class="button"><?php _e('Edit', 'coworking-text-domain'); ?></a>
                    <form method="post" style="display:inline;">
                        <?php wp_nonce_field('manage_company_location_nonce', 'company_location_nonce'); ?>
                        <input type="hidden" name="company_id"
                            value="<?php echo esc_attr($company_location->company_id); ?>">
                        <input type="hidden" name="location_id"
                            value="<?php echo esc_attr($company_location->location_id); ?>">
                        <input type="submit" name="delete_company_location"
                            value="<?php _e('Delete', 'coworking-text-domain'); ?>" class="button button-danger">
                    </form>
                </td>
            </tr>
        <?php endforeach; ?>
